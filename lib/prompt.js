...
export function buildSystem(tone) {
  ...
  return [
    ...
    "",
    `Return exactly ${VARIANTS} rewrites that differ STRUCTURALLY from one another,`,
    "not merely by swapping synonyms. Vary sentence structure, clause order, voice",
    "and length between them. Two rewrites that differ only by a word or two are a",
    "failure: replace one with a genuinely different construction.",
